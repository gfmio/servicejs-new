...
/**
 * Bootstrap the Web Worker runtime with all capabilities
 */
export function bootstrap(
  options: WebWorkerBootstrapOptions = {}
): WebWorkerRuntimeCapabilities {
  const {
    config = {},
  ...
  // Environment capability (uses config passed from main thread)
  const env: EnvironmentCapability = {
    ...
    platform: 'web-worker' as Platform,
    version: '1.0.0',
  };
  ...
  // Message Port capability (for communication with main thread)
  const messageHandlers: Array<(event: MessageEvent) => void> = [];

  const messagePort: MessagePortCapability = {
