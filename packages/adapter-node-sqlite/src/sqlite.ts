/**
 * Node.js SQLite Database Adapter
 *
 * Real implementation using better-sqlite3
 */
    ...
    try {
      ...
      if (isSelect) {
        if (query.params && query.params.length > 0) {
          rows = stmt.all(...query.params) as TRow[];
        } else {
          rows = stmt.all() as TRow[];
        }
        rowCount = rows.length;
      } else {
        // For INSERT/UPDATE/DELETE, run the query and return changes count
        let info: Database.RunResult;
        if (query.params && query.params.length > 0) {
          info = stmt.run(...query.params);
        } else {
          info = stmt.run();
        }
        rows = [];
